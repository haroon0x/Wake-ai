...
interface AgentTaskDao {
    ...
    @Query("SELECT * FROM agent_task WHERE status IN ('watching', 'thinking', 'proposed') AND type = :type AND pkg = :pkg AND sender = :sender AND (:conversationId IS NULL OR conversationId = :conversationId) LIMIT 1")
    suspend fun activeFor(type: String, pkg: String?, sender: String?, conversationId: String?): AgentTask?
